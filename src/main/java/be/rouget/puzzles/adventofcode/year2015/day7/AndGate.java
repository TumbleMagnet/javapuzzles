...
@Value
public class AndGate implements Connection {
    Input input1;
    Input input2;
    ...
    public static AndGate fromInput(String inputText) {
        Pattern pattern = Pattern.compile("(.*) AND (.*)");
        Matcher matcher = pattern.matcher(inputText);
        if (!matcher.matches()) {
            throw new IllegalArgumentException("Cannot parse input: " + inputText);
        }
        return new AndGate(Circuit.parseInput(matcher.group(1)), Circuit.parseInput(matcher.group(2)));
    }
}
