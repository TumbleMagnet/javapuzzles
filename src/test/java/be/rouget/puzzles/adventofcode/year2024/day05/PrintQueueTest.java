package be.rouget.puzzles.adventofcode.year2024.day05;

import org.junit.jupiter.api.Test;

import java.util.List;

import static org.assertj.core.api.Assertions.assertThat;

class PrintQueueTest {

    public static final List<String> TEST_INPUT = List.of(
            "47|53",
            "97|13",
            "97|61",
            "97|47",
            "75|29",
            "61|13",
            "75|53",
            "29|13",
            "97|29",
            "53|29",
            "61|53",
            "97|53",
            "61|29",
            "47|13",
            "75|47",
            "97|75",
            "47|61",
            "75|61",
            "47|29",
            "75|13",
            "53|13",
            "",
            "75,47,61,53,29",
            "97,61,53,29,13",
            "75,29,13",
            "75,97,47,61,53",
            "61,13,29",
            "97,13,75,29,47"
    );


    @Test
    void computeResultForPart1() {
        PrintQueue solver = new PrintQueue(TEST_INPUT);
        assertThat(solver.computeResultForPart1()).isEqualTo(143L);
    }

    @Test
    void computeResultForPart2() {
        PrintQueue solver = new PrintQueue(TEST_INPUT);
        assertThat(solver.computeResultForPart2()).isEqualTo(123L);
    }
}